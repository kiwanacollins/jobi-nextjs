...
const SearchItems = () => {
  ...
  const [jobs, setJobs] = useState<IJobData[]>(allJobData);
  ...
  const categoryMatch = (item: IJobData) => {
    // return item.category.some(
    //   (e) => slugify(e.split(',').join('-').toLowerCase(), '-') === category
    // );
    const slugifiedItemCategory = slugify(item.category.toLowerCase(), '-');
    return slugifiedItemCategory === category;
  };
